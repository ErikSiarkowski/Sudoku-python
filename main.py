...
def Botao_Preencher(window):
    pg.draw.rect(window, azul, (700, 175, 250, 100), border_radius=15 )
    palavra_f = fonte.render('Fill', True, preto)
    window.blit(palavra_f, (760, 200))
...
def Click_Botao_Preencher(mouse_position_x, mouse_position_y, click_last_status, click, tabuleiro_data, jogo_data):
    x = mouse_position_x
    y = mouse_position_y
    if x >= 700 and x <= 950 and y >= 175 and y <= 275 and click_last_status == False and click == True:
        jogo_data = tabuleiro_data
    return  jogo_data
